...
func Throttle(limit int) func(ctx *gin.Context) {
...
// pending requests.
func ThrottleBacklog(limit int, backlogLimit int, backlogTimeout time.Duration) func(ctx *gin.Context) {
...
func ThrottleWithOpts(opts ThrottleOpts) func(ctx *gin.Context) {
	if opts.Limit < 1 {
	...
	return func(c *gin.Context) {
		...
		select {
		case <-ctx.Done():
			t.setRetryAfterHeaderIfNeeded(c.Writer, true)
			http.Error(c.Writer, errContextCanceled, http.StatusServiceUnavailable)
			c.Abort()
			return
		...
		case btok := <-t.backlogTokens:
			...
			select {
			case <-timer.C:
				t.setRetryAfterHeaderIfNeeded(c.Writer, false)
				http.Error(c.Writer, errTimedOut, http.StatusServiceUnavailable)
				c.Abort()
				return
			case <-ctx.Done():
				timer.Stop()
		...
		default:
			t.setRetryAfterHeaderIfNeeded(c.Writer, false)
			http.Error(c.Writer, errCapacityExceeded, http.StatusServiceUnavailable)
			c.Abort()
			return
		}
	}
