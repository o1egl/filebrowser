...
func TestThrottleBacklog(t *testing.T) {
	...
	for i := 0; i < 40; i++ {
		...
		go func(i int) {
			...
			require.NoError(t, err)
			require.Equal(t, testContent, buf)
		}(i)
	}
...
func TestThrottleTriggerGatewayTimeout(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		go func(i int) {
			...
			require.Equal(t, errTimedOut, strings.TrimSpace(string(buf)))
...
func TestThrottleMaximum(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func(i int) {
			...
			require.NoError(t, err)
			require.Equal(t, http.StatusServiceUnavailable, res.StatusCode)
			require.Equal(t, errCapacityExceeded, strings.TrimSpace(string(buf)))
...
func TestThrottleRetryAfter(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(i int) {
			...
			require.NoError(t, err)
			require.Equal(t, http.StatusServiceUnavailable, res.StatusCode)
			require.Equal(t, res.Header.Get("Retry-After"), "3600")
		}(i)
	}
