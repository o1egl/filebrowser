...
import (
	...
	mock "github.com/filebrowser/filebrowser/log/mock"
)
...
func TestToStdLogger(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"debug": {
			mockInit: func(t *testing.T, loggerMock *mock.MockLogger) {
				loggerMock.EXPECT().Debug(expectMsg)
			},
			level: log.LevelDebug,
		},
		"info": {
			mockInit: func(t *testing.T, loggerMock *mock.MockLogger) {
				loggerMock.EXPECT().Info(expectMsg)
			},
			level: log.LevelInfo,
		},
		"warning": {
			mockInit: func(t *testing.T, loggerMock *mock.MockLogger) {
				loggerMock.EXPECT().Warn(expectMsg)
			},
			level: log.LevelWarn,
		},
		"error": {
			mockInit: func(t *testing.T, loggerMock *mock.MockLogger) {
				loggerMock.EXPECT().Error(expectMsg)
			},
			level: log.LevelError,
		},
		"fatal": {
			mockInit: func(t *testing.T, loggerMock *mock.MockLogger) {
				loggerMock.EXPECT().Fatal(expectMsg)
			},
