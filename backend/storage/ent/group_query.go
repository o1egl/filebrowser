...
func (gq *GroupQuery) QueryVolumes() *VolumeQuery {
	query := (&VolumeClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) QueryGroupVolume() *GroupVolumeQuery {
	query := (&GroupVolumeClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withGroupVolume: gq.withGroupVolume.Clone(),
...
func (gq *GroupQuery) WithGroupVolume(opts ...func(*GroupVolumeQuery)) *GroupQuery {
	query := (&GroupVolumeClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withGroupVolume = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withVolumes; query != nil {
		if err := gq.loadVolumes(ctx, query, nodes,
			func(n *Group) { n.Edges.Volumes = []*Volume{} },
			func(n *Group, e *Volume) { n.Edges.Volumes = append(n.Edges.Volumes, e) }); err != nil {
			return nil, err
		}
	}
	if query := gq.withGroupVolume; query != nil {
		if err := gq.loadGroupVolume(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupVolume = []*GroupVolume{} },
			func(n *Group, e *GroupVolume) { n.Edges.GroupVolume = append(n.Edges.GroupVolume, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadVolumes(ctx context.Context, query *VolumeQuery, nodes []*Group, init func(*Group), assign func(*Group, *Volume)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Group)
	nids := make(map[int]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.VolumesTable)
		s.Join(joinT).On(s.C(volume.FieldID), joinT.C(group.VolumesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.VolumesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.VolumesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Volume](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "volumes" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (gq *GroupQuery) loadGroupVolume(ctx context.Context, query *GroupVolumeQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupVolume)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(groupvolume.FieldGroupID)
	}
	query.Where(predicate.GroupVolume(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.GroupVolumeColumn), fks...))
	}))
